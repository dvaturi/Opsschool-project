...
resource "aws_route53_record" "consul" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.consul_server.private_ip}:${aws_lb_target_group.kibana_server.port}"]
}
...
resource "aws_route53_record" "jenkins" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.jenkins_server.private_ip}:${aws_lb_target_group.kibana_server.port}"]
}
...
resource "aws_route53_record" "prometheus" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.prometheus_server.private_ip}:${aws_lb_target_group.kibana_server.port}"]
}
